...
public:
    ...
    {
        // tunable parameters (change with ros2 param set)
        ...
        RCLCPP_INFO(get_logger(), "lidar_tree_detector started (scan=%s)", scan_topic_.c_str());
...
private:
    ...
    {
        load_parameters(); // pick up runtime param changes
        ...
        for (const auto &cluster : clusters) {
            ...
            // compute perimeter (convex hull) and fit circle to hull (impose circle on perimeter)
            auto hull = convex_hull(cluster);
            const auto &fit_pts = (hull.size() >= 3) ? hull : cluster; // prefer hull, fallback to full cluster

            double cx=0.0, cy=0.0, radius=0.0, rmse=0.0;
            bool fit_ok = fit_circle_kasa(fit_pts, cx, cy, radius, rmse);

            if (debug_reasons_) {
                RCLCPP_DEBUG(get_logger(), "cluster pts=%zu hull=%zu fit_ok=%d rmse=%.4f r=%.3f",
                             cluster.size(), hull.size(), static_cast<int>(fit_ok), rmse, radius);
            }

            bool accepted = false;
            double diameter = 0.0;
            if (fit_ok && rmse <= fit_max_rmse_ && radius >= fit_min_radius_ && radius <= fit_max_radius_) {
                diameter = 2.0 * radius;
                accepted = true;
            } else {
                if (debug_reasons_) {
                    if (fit_ok) RCLCPP_DEBUG(get_logger(), "Fit outside bounds rmse=%.4f r=%.3f", rmse, radius);
                    else RCLCPP_DEBUG(get_logger(), "Fit failed");
                }
                // fallback: centroid + max pairwise on cluster perimeter (use hull if available)
                auto cen = compute_centroid(fit_pts);
                double fallback_diam = estimate_diameter(fit_pts);
            ...
            int idx = find_matching_tree(cx, cy);
            if (idx >= 0) {
                Tree &t = trees_[idx];
                t.count++;
    ...
    // convert scan to xy points, filter by max_distance_
    ...
    // simple sequential clustering using angular ordering
    ...
    // monotone chain convex hull (returns hull in CCW, no duplicate endpoints)
    static std::vector<std::pair<double,double>> convex_hull(const std::vector<std::pair<double,double>>& pts)
    {
        std::vector<std::pair<double,double>> pts_sorted = pts;
        ...
        std::vector<std::pair<double,double>> hull = lower;
        hull.insert(hull.end(), upper.begin(), upper.end());
        return hull;
    }

    // compute chord / max perpendicular for curvature test (optional)
    ...
    // solve 3x3 augmented system a[3][4] -> x[3]
    ...
    // Kåsa least-squares circle fit (algebraic)
